...
#for(cat in 1:5){
  categoria <- names(main_dict)[1]
  cat_tot <- get_category_total(categoria,person)
  text(-1.5,y_coords_category[1],
       categoria)
  text(x_coord_category,y_coords_category[2],
...
#}
#for(dom in 1:10){
  dominio <- names(inverse_dict)[4]
  dom_tot <- get_domain_total(dominio,person)
  text(.85,y_coords_domain[4],
       dominio)
  text(x_coord_domain,y_coords_domain[4],
